...
export default function CheckoutSummary({ cart, totaleProdotti, costoSpedizione, totaleFinale, isSpedizioneGratuita, termsAccepted, handleSubmit }) {
    ...
    return (
        <div className="bg-white p-4 rounded shadow-sm sticky-top border border-light" style={{ top: '20px' }}>
                    ...
                    {cart.map((item) => (
                        <li key={item.product_id} className="d-flex justify-content-between align-items-center mb-3">
                            <div className="d-flex align-items-center" style={{ maxWidth: '75%' }}>
                                <img src={item.img_url} alt={item.name} className="rounded shadow-sm me-3" style={{ width: '45px', height: '45px', objectFit: 'cover' }} />
                                <div className="text-truncate">
                                    <h6 className="mb-0 fw-bold text-blue small text-truncate">{item.name}</h6>
                                    <small className="text-muted">{item.quantity}x - {item.artist_name}</small>
                                </div>
                            </div>
            ...
            <button
                className="btn bg-yellow text-blue w-100 fw-bold py-3 shadow-sm border-0 text-uppercase"
